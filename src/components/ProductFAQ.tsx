...
interface Props {
  ...
  items: FAQItem[];
  accentColor?: string;
  /** Unique ID for per-page FAQPage JSON-LD schema (e.g., "ferro-alloys-faq") — enables page-level schema matching */
  faqPageId?: string;
}

export default function ProductFAQ({ title = "Frequently Asked Questions", subtitle, items, accentColor = "#DCA54C", faqPageId }: Props) {
  const [activeIndex, setActiveIndex] = useState<number | null>(0);

  // Generate per-page FAQPage JSON-LD for stronger AEO/GEO signal
  const faqSchema = faqPageId ? {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    "@id": `https://lauls.in/#${faqPageId}`,
